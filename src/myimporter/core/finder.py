...
from src.myimporter.utils.trace import trace, TraceScope
from src.myimporter.utils.settings import MYIMPORTER_DEBUG_ENVAR_NAME, MYIMPORTER_TRACE_ENVAR_NAME, FINDER_CUSTOMFINDER_FIND_SPEC_EXCLUDED_MODULES
...
class CustomFinder(importlib.abc.MetaPathFinder):
    ...
    def find_spec(self, fullname, path=None, target=None):
        ...
        if fullname in sys.modules:
            logger.warning(f"[CustomFinder] Module {fullname} already in sys.modules, skipping CustomFinder")
            return None

        # 检查内置模块，内置模块不应该由CustomFinder处理，这也是Python导入系统的标准行为
        if fullname in sys.builtin_module_names or fullname.startswith(FINDER_CUSTOMFINDER_FIND_SPEC_EXCLUDED_MODULES):
        ...
        logger.warning(f"[CustomFinder] Module {fullname} not found by any provider, will leave it to python's default import system")
        return None
