...
python projects\localization_resource_auditor\localization_auditor.py --format json --observe
```

指定外部文件：

```powershell
python projects\localization_resource_auditor\localization_auditor.py --source path\to\source.json --target path\to\target.json --glossary path\to\glossary.csv --max-length 42
python projects\localization_resource_auditor\localization_auditor.py --format json --output report.json
```
